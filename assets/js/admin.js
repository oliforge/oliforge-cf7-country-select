(() => {
  const root = document.querySelector('.oliforge-country-settings');
  if (!root) return;
  ...
  const search = root.querySelector('#oliforge-country-settings-search');
  const count = root.querySelector('.oliforge-country-settings__count');
  const updateCount = () => { count.textContent = `${boxes.filter((box) => box.checked).length} / ${boxes.length}`; };
  root.querySelector('[data-oliforge-select-all]').addEventListener('click', () => { boxes.forEach((box) => { box.checked = true; }); updateCount(); });
  root.querySelector('[data-oliforge-clear-all]').addEventListener('click', () => { boxes.forEach((box) => { box.checked = false; }); updateCount(); });
